    ...
    if params[:page].to_s.empty? || params[:limit].to_s.empty? #リクエスト不備の時エラーを返す
      ...
      if params[:tag] #ーーーーーータグ指定ありーーーーーーー
        ...
        @screenshots.each do|screenshot|
          ...
          screenshot_main_tag = screenshot.tag.as_json(root: "maintag") #hash1
          ...
          userflow_maintag = screenshot.user_flow.tag.as_json(root:"maintag")
        ...
        @screenshots.each do|screenshot|
          #:screenshot =>　maintag, tags           
          screenshot_main_tag = screenshot.tags.find_by(id: screenshot.tag_id).as_json(root: "maintag") #hash1
          ...
          #:userflow => maintag, product, platform
          userflow = screenshot.user_flow.as_json(include: [{product:{only:[:id,:name, :description, :slug, :icon_path]}},{platform:{only:[:id,:name, :slug]}},:tags])
          userflow_maintag = screenshot.user_flow.tags.find_by(id:screenshot.user_flow.tag_id).as_json(root: "maintag")
          userflow_with_maintag = {userflow: userflow.merge(userflow_maintag)}
